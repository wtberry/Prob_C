#ifndef ANGLE_H
#define ANGLE_H
#include<iostream>
#include"Log.h"
#include"Point.h"
#include<cmath>
#include<cstdio>

#define ROUND 360


class Point;

template <class T>class Angle{

private:
	T angle; // generic class ish
	mutable double sin_val;
	mutable double cos_val;
	mutable bool have_sin;
	mutable bool have_cos;

public:

	// std::absolute value and %
	Angle(){
		// empty const, initialize angle to 0
		this->angle = 0;
		this->sin_val = 0;
		this->cos_val = 0;
		resetTrig();
	
	}
	
	Angle(T angle){
		// standard const with input
		// make sure the angle is correct ranage
		this->angle = this->resetAngle(angle);
		this->sin_val = 0;
		this->cos_val = 0;
		resetTrig();
	}
	
	~Angle(){ // empty destructor for now
	}
	
	// getter
	T getAngle() const{
		// return value angle
		return angle;
	}
	
	double getSin() const{
		// check if calculated before
		if(!have_sin){
			sin_val = sin(this->angle*M_PI/180.0);
			have_sin = true;
		}
		return sin_val;
	}
	
	
	double getCos() const{
		// check if calculated before
		if(!have_cos){
			cos_val = cos(this->angle*M_PI/180.0);
			have_cos = true;
		}
		return cos_val;
	}
	// setter
	void setAngle(T angle) { // modify the angle, so no const
		// set new angle
		this->angle = resetAngle(angle);
		resetTrig();
	}
	
	double resetAngle(T angle) const{
		// use std::abs() instead of just abs(), to return abs as double
		int tmp = angle/ROUND; // integer portion of the quotient
		double remain = std::abs(angle) - std::abs(tmp * ROUND); // remainder
		if ((angle>=0) || (std::abs(angle)==ROUND)){
			return remain;
		}
		else{
			return -remain + ROUND;
		}
	}
	
	// set all the trig bol to false
	void resetTrig(){
		this->have_sin = false;
		this->have_cos = false;
	}
	
	void arnold() const{
		std::cout << "Division by 0,  terminating the program1n" << std::endl;
		exit(1);
	}
	
	
	// overload`
	Angle operator+(const Angle& angle) const{
		// why can I access angle by angle.angle, not getAngle? thought it was private?
		return Angle(this->resetAngle((this->angle + angle.angle)));
	}
	
	Angle operator+=(const Angle& angle) {
		// why can I access angle by angle.angle, not getAngle? thought it was private?
		this->angle += angle.angle;
		this->angle = resetAngle(this->angle);
		resetTrig();
	}
	
	Angle operator-(const Angle& angle) const{
		// why can I access angle by angle.angle, not getAngle? thought it was private?
		return (*this)+(-angle.getAngle()); // using + method for -
	}
	
	Angle operator-=(const Angle& angle) {
		// why can I access angle by angle.angle, not getAngle? thought it was private?
		this->angle -= angle.angle;
		this->angle = resetAngle(this->angle);
		resetTrig();
	}
	//Angle ngle::operator-(const Angle& angle) const{
	//	return Angle(this->resetAngle(this->angle - angle.angle));
	//}
	
	Angle operator*(const Angle& angle) const{
		return Angle(this->resetAngle(this->angle * angle.angle));
	}
	
	Angle operator*=(const Angle& angle) {
		// why can I access angle by angle.angle, not getAngle? thought it was private?
		this->angle *= angle.angle;
		this->angle = resetAngle(this->angle);
		resetTrig();
	}
	Angle operator/(const Angle& angle) const{
		// make sure no zero division
		if(angle.angle == 0) arnold();
		else{
		return Angle(this->resetAngle(this->angle / angle.angle));
		}
	}
	
	Angle operator/=(const Angle& angle) {
		// why can I access angle by angle.angle, not getAngle? thought it was private?
		if(angle.angle == 0) arnold();
		this->angle /= angle.angle;
		this->angle = resetAngle(this->angle);
		resetTrig();
	}
	
	void operator=(const Angle& angle){
		this->angle = this->resetAngle(angle.angle);
		resetTrig();
	}
	
	void operator=(const double angle){
		this->angle = this->resetAngle(angle);
	}
	bool operator==(const Angle& angle)const{
		if(this->angle == angle.angle) return true;
		else return false;
	}
	
	// now irintable, why tf does this work with frind keyword?
	friend std::ostream& operator<<(std::ostream& out, const Angle<T>& a){
		out << a.angle << "\u00B0";
		return out;
	}
	// end of the class


}; // semicolon don't forget

//std::ostream& operator<<(std::ostream& out, const Angle& a);

#endif
